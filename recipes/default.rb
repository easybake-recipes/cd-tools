...
when /debian/
  %w{ttf-dejavu ttf-dejavu-core ttf-dejavu-extra zip}
else
...
execute 'npm install jslint -g' do
  creates '/usr/local/bin/jslint'
end

execute 'npm install jsonlint -g' do
  creates '/usr/local/bin/jsonlint'
end
...
execute "ssh-keygen -q -f /var/lib/jenkins/.ssh/id_rsa -P ''" do
  user 'jenkins'
  group 'jenkins'
  not_if {File.exists? "/var/lib/jenkins/.ssh/id_rsa.pub"}
end

execute "ssh #{node['cd-tools']['gerrit']['admin_user']}@#{node['gerrit']['http_proxy']['host_name']} -o StrictHostKeyChecking=no -p 29418" do
  user 'jenkins'
  group 'jenkins'
  ignore_failure true
  not_if {File.exists? "/var/lib/jenkins/.ssh/authorized_keys"}
end

execute "ssh-keygen -q -f /srv/gerrit/site_path/.ssh/id_rsa -P ''" do
  user 'gerrit2'
  group 'gerrit2'
  not_if {File.exists? "/srv/gerrit/site_path/.ssh/id_rsa.pub"}
end
